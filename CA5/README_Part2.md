...
The Dockerfile is an essential part of the pipeline, used to build a Docker image that contains your Spring Boot application running on a Tomcat server. Here's what each part of the Dockerfile does:
...
EXPOSE 8080
```
- **Base Image**: Starts from the `tomcat:9.0-jdk11-adoptopenjdk-hotspot` image which includes Tomcat 9.0 with JDK 11.
- **Remove Default ROOT**: Deletes the default ROOT application to make room for your application.
- **Copy WAR File**: Copies the WAR file generated by the build process into the Tomcat webapps directory, renaming it to `ROOT.war` so it becomes the default application.
- **Expose Port 8080**: Exposes port 8080 to allow access to the Tomcat server.

### 4. Jenkins Pipeline Configuration

The following Jenkinsfile defines the steps for the Jenkins pipeline:
